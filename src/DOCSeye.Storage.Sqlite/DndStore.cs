...
public sealed class DndStore : IDisposable
{
    ...
    public static DndStore Create(string path,SemanticState state,IReadOnlyList<Guid>? parents=null)
    {
        ...
        foreach(var o in state.Objects.Values)s.InsertObject(tx,o);foreach(var b in state.Boundaries.Values)s.InsertBoundary(tx,b);foreach(var e in state.Extensions.Values)s.InsertExtension(tx,e);foreach(var a in state.Assets.Values)s.InsertAsset(tx,a);foreach(var f in state.ProviderFacets.Values)s.InsertProviderFacet(tx,f);foreach(var capsule in state.SourceCapsules.Values)s.InsertSourceCapsule(tx,capsule);foreach(var w in state.Retired.Values)s.InsertRetired(tx,w);
