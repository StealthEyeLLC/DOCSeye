...
CREATE TABLE retained_ranges(row_no INTEGER PRIMARY KEY AUTOINCREMENT,id BLOB NOT NULL,start_boundary_id BLOB NOT NULL,end_boundary_id BLOB NOT NULL,allow_multi_interval INTEGER NOT NULL,kind TEXT NOT NULL,state TEXT NOT NULL,intervals_cbor BLOB NOT NULL,cbor BLOB NOT NULL);
...
";cmd.ExecuteNonQuery();
    ...
    public SemanticState LoadState()
    {
        ...
        using(var cmd=c.CreateCommand()){cmd.CommandText="SELECT id,start_boundary_id,end_boundary_id,allow_multi_interval,kind,state,intervals_cbor FROM retained_ranges ORDER BY row_no";using var r=cmd.ExecuteReader();while(r.Read()){Guid id=Ids.GuidFromRfcBytes((byte[])r[0]);s.Ranges[id]=new(id,Ids.GuidFromRfcBytes((byte[])r[1]),Ids.GuidFromRfcBytes((byte[])r[2]),r.GetInt64(3)!=0,r.GetString(4),r.GetString(5)){Intervals=ParseIntervals((byte[])r[6])};}}
    ...
    private static byte[] EncodeIntervals(RetainedRange r)=>CanonicalCbor.Encode(r.EffectiveIntervals.Select(i=>(object?)new object?[]{i.StartBoundaryId,i.EndBoundaryId}).ToArray());
    private static IReadOnlyList<RangeInterval> ParseIntervals(byte[] bytes)
    {
        var outer=(object?[])(CanonicalCbor.Decode(bytes)??Array.Empty<object?>());var result=new List<RangeInterval>();
        foreach(var item in outer){var pair=(object?[])item!;if(pair.Length!=2||pair[0] is not Guid a||pair[1] is not Guid b)throw new InvalidOperationException("invalid range interval encoding");result.Add(new(a,b));}
        return result;
    }
    ...
    private void InsertRange(SqliteTransaction tx,RetainedRange r)=>c.Exec(tx,"INSERT INTO retained_ranges(id,start_boundary_id,end_boundary_id,allow_multi_interval,kind,state,intervals_cbor,cbor) VALUES($i,$s,$e,$m,$k,$st,$ic,$c)",("$i",Ids.RfcBytes(r.Id)),("$s",Ids.RfcBytes(r.StartBoundaryId)),("$e",Ids.RfcBytes(r.EndBoundaryId)),("$m",r.AllowMultiInterval?1:0),("$k",r.Kind),("$st",r.State),("$ic",EncodeIntervals(r)),("$c",CanonicalCbor.EncodeRange(r)));
    ...
    public ValidationReport Validate(bool verifyAssets=true)
    {
        var d=new List<string>();try
        {
            ...
            using(var cmd=c.CreateCommand()){cmd.CommandText="SELECT id,start_boundary_id,end_boundary_id,allow_multi_interval,intervals_cbor FROM retained_ranges";using var r=cmd.ExecuteReader();while(r.Read()){Guid id=Ids.GuidFromRfcBytes((byte[])r[0]);if(!Ids.IsV4(id))return new(false,"invalid_identity",3,[$"range {id} not UUIDv4"]);var intervals=ParseIntervals((byte[])r[4]);if(intervals.Count==0||(!r.GetBoolean(3)&&intervals.Count!=1))return new(false,"invalid_range",4,[$"range {id} interval cardinality invalid"]);foreach(var interval in intervals)foreach(Guid bid in new[]{interval.StartBoundaryId,interval.EndBoundaryId})if(Convert.ToInt64(c.Scalar(null,"SELECT count(*) FROM boundaries WHERE id=$i",("$i",Ids.RfcBytes(bid))))!=1)return new(false,"broken_reference",3,[$"range {id} boundary missing"]);}}
