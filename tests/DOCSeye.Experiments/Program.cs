...
void RunE11()
{
    ...
    Directory.Delete(runtimeDir,true);Expect(!Directory.Exists(runtimeDir),"runtime state deletion failed");Directory.CreateDirectory(runtimeDir);using(var store=DndStore.Open(scale500Path!)){var afterDelete=store.ReadHead();Expect(afterDelete.RevisionId==before.RevisionId&&afterDelete.SemanticRoot.SequenceEqual(before.SemanticRoot),"runtime loss changed native truth");using(var idx=new RuntimeIndex(index)){idx.RebuildFrom(store);Expect(idx.Search(token).Contains(target),"rebuilt runtime index lost semantic identity");}store.PruneExpiredWitnesses(afterDelete.Sequence+1);var delta=store.ReadDeltas(0);Expect(delta.ResyncRequired&&delta.Classification=="cursor_expired","expired delta cursor did not require resync");Expect(store.ReadHead().SemanticRoot.SequenceEqual(before.SemanticRoot),"delta witness pruning changed semantic root");}
    results.Add(new{id="E-11",result="PASS",runtime_state_deleted_completely=true,index_rebuilt_from_dnd=true,semantic_identity_exact=true,semantic_root_unchanged=true,expired_cursor_classification="cursor_expired",resync_required=true,external_state_is_truth=false});Console.WriteLine("E-11 PASS");
}
